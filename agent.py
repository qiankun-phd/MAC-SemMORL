...
try:
    ...
    get_performance_indicator = None
...
class SacAgent:
    ...
    def __init__(self, env_id, env, log_dir, num_steps=3000000, batch_size=256,
        ...
        if self.env.reward_num == 2:
            ...
            step = {3: 0.05, 4: 0.1, 5: 0.2}.get(self.env.reward_num, 0.1)
            try:
                PREF_ = np.load(f"{self.env.reward_num}pref_table.npy")
            except FileNotFoundError:
                PREF_ = generate_w_batch_test(self.env.reward_num, step)
        for i in PREF_:
            self.tot_t.append([])
    ...
    def get_pref(self):
        ...
        if self.fixed_weight is not None:
            return np.array(self.fixed_weight, dtype=np.float32)
    ...
    def run(self,our_wandb):
        step_map = {2: 0.005, 3: 0.1, 4: 0.2, 5: 0.25, 6: 0.3}
        step_size = step_map.get(self.env.reward_num)
        assert step_size is not None, f"Unsupported reward_num={self.env.reward_num}"
        Preference_for_HV = generate_w_batch_test(self.env.reward_num, step_size=step_size)
        print(f"Preference_for_HV: {len(Preference_for_HV)} preferences (step={step_size})", flush=True)
    ...
    def train_episode(self,Preference_for_HV):
        ...
        if self.steps - self.previous_save >= self.model_saved_step:
            ckpt_dir = os.path.join(self.log_dir, "checkpoints")
            os.makedirs(ckpt_dir, exist_ok=True)
            tag = str(int(self.steps // self.model_saved_step))
            self.previous_save = self.steps
            torch.save(self.policy.state_dict(), os.path.join(ckpt_dir, f"policy_{tag}.pkl"))
            torch.save(self.latent_encoder.state_dict(), os.path.join(ckpt_dir, f"encoder_{tag}.pkl"))
            torch.save(self.critic.state_dict(), os.path.join(ckpt_dir, f"critic_{tag}.pkl"))
            print(f"  [checkpoint saved at step {self.steps}]", flush=True)
        ...
        if self.steps - self.previous_evluate >= self.eval_interval:
            ...
            print(f"  [eval step={self.steps}] HV={hv:.1f}  UT={UT:.1f}  Sparsity={sparsity:.1f}  "
                  f"mean_obj={objs.mean(axis=0)}", flush=True)
        ...
        if self.steps > self.num_steps:
            ...
            print(f"  [FINAL eval step={self.steps}] HV={hv:.1f}  UT={UT:.1f}  Sparsity={sparsity:.1f}", flush=True)

            ckpt_dir = os.path.join(self.log_dir, "checkpoints")
            os.makedirs(ckpt_dir, exist_ok=True)
            torch.save(self.policy.state_dict(), os.path.join(ckpt_dir, "policy_final.pkl"))
            torch.save(self.latent_encoder.state_dict(), os.path.join(ckpt_dir, "encoder_final.pkl"))
            torch.save(self.critic.state_dict(), os.path.join(ckpt_dir, "critic_final.pkl"))
            print(f"  [final checkpoint saved]", flush=True)
        
        print(f'episode: {self.episodes:<4}  '
              f'episode steps: {episode_steps:<4}  '
              f'episode weight: {preference}  '
              f'rl reward:', rl_reward, " cost avg time", (time.time() - start)/episode_steps, flush=True)
    ...
    def learn(self,fixed_preference, other_fixed_preference):
        ...
        temp_critic_state = {k: v.clone() for k, v in self.critic.state_dict().items()}
        ...
        if self.learning_steps % self.old_Q_update_freq==0:
            self.old_critic.load_state_dict(temp_critic_state)
    ...
    def calc_critic_loss(self , s_z, batch, weights, preference, PREF, best_policy, best_policy_weight):
        ...
        sampled_preference_without_repeat = torch.as_tensor(self.get_pref(), dtype=torch.float32, device=self.device)
        sampled_preference = sampled_preference_without_repeat.detach().clone().repeat(states.shape[0], 1)
        ...
        s_q1 = stiffness_Q1.item() if torch.is_tensor(stiffness_Q1) else float(stiffness_Q1)
        s_q2 = stiffness_Q2.item() if torch.is_tensor(stiffness_Q2) else float(stiffness_Q2)
        q1_total_loss = q1_loss + max(self.regular_bar - s_q1, 0.0) * self.regular_alpha * regular_q1
        q2_total_loss = q2_loss + max(self.regular_bar - s_q2, 0.0) * self.regular_alpha * regular_q2
    ...
    def calc_policy_loss(self, current_latent,  batch, weights, preference, PREF):
        ...
        for a, c in enumerate([ self.critic]+self.Q_memory.sample() ): # Use critic from Q Replay Buffer
            for b, i in enumerate(PREF): #Get Q from preference set W
                p_batch = torch.as_tensor(i, dtype=torch.float32, device=self.device).detach().clone().repeat(self.batch_size, 1)
    ...
    def get_objs(self, Preference_for_HV, eval_episodes=1):
        recovered_objs = np.zeros((len(Preference_for_HV), self.env.reward_num))
        comm_metrics = {
            "weighted_avg_fidelity": np.zeros(len(Preference_for_HV)),
            "mean_aosi": np.zeros(len(Preference_for_HV)),
            "energy": np.zeros(len(Preference_for_HV)),
            "jain_fairness": np.zeros(len(Preference_for_HV)),
            "service_rate": np.zeros(len(Preference_for_HV)),
        }
        for eval_ep in range(eval_episodes):
            for p_index, evalPreference in enumerate(Preference_for_HV):
                ...
                tot_rewards = 0
                ep_steps = 0
                ep_fid, ep_aosi, ep_energy, ep_jain, ep_sr = 0, 0, 0, 0, 0
                while not terminal:
                    action = self.exploit(state, evalPreference)
                    next_state, reward, terminal, info = self.env.step(action * self.max_action)
                    tot_rewards += reward
                    ep_fid += info.get("weighted_avg_fidelity", 0)
                    ep_aosi += info.get("mean_aosi", 0)
                    ep_energy += info.get("energy", 0)
                    ep_jain += info.get("jain_fairness", 0)
                    ep_sr += info.get("service_rate", 0)
                    ep_steps += 1
                    state = next_state
                recovered_objs[p_index] += tot_rewards
                comm_metrics["weighted_avg_fidelity"][p_index] += ep_fid / max(ep_steps, 1)
                comm_metrics["mean_aosi"][p_index] += ep_aosi / max(ep_steps, 1)
                comm_metrics["energy"][p_index] += ep_energy
                comm_metrics["jain_fairness"][p_index] += ep_jain / max(ep_steps, 1)
                comm_metrics["service_rate"][p_index] += ep_sr / max(ep_steps, 1)
        recovered_objs /= eval_episodes
        for k in comm_metrics:
            comm_metrics[k] /= eval_episodes
        np.savez(
            os.path.join(self.summary_dir, f"comm_metrics_{self.steps}.npz"),
            **comm_metrics,
        )
