...
class Conflict_caculate():
    ...
    def get_stiffness(self, grads):
        ...
        g0 = grads_0.detach().cpu().float()
        g1 = grads_1.detach().cpu().float()
        g0 = torch.nan_to_num(g0, nan=0.0, posinf=1e6, neginf=-1e6)
        g1 = torch.nan_to_num(g1, nan=0.0, posinf=1e6, neginf=-1e6)
        norm_prod = (g0.norm() * g1.norm()).clamp(min=1e-8)
        return torch.dot(g0, g1) / norm_prod
