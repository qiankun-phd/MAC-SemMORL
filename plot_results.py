"""Generate IEEE-compliant publication figures for GLOBECOM paper.
...
SEEDS = {
    "COLA":     ["cola_full_s1", "cola_full_s2", "cola_full_s3"],
    "No COR":   ["no_cor_s1",   "no_cor_s2",   "no_cor_s3"],
    "No OADM":  ["no_oadm_s1",  "no_oadm_s2",  "no_oadm_s3"],
    "Envelope": ["envelope_s1", "envelope_s2", "envelope_s3"],
}


def legend_label(method_key):
    """Paper-facing legend name (internal keys unchanged for log paths)."""
    return "Envelope SAC" if method_key == "Envelope" else method_key


def comm_legend_label(method_key):
    """Short names for panel (b) method legend."""
    short = {
        "WS-Fidelity": "WS-Fid.",
        "WS-Balanced": "WS-Bal.",
        "WS-Energy": "WS-Eng.",
    ...
    return short.get(method_key, legend_label(method_key))
WS_EXPS = {
...
for method, exp in WS_EXPS.items():
    st, rw = load_training_curve(exp)
    for obj_i in range(N_OBJ):
        ax = axes[obj_i // 2][obj_i % 2]
        y = smooth(rw[:, obj_i])
        x = np.linspace(0, st[-1], len(y))
        ax.plot(x / 1000, y, color=COLORS[method], label=method,
...
method_display = [
    "COLA", "w/o COR", "w/o OADM", "Envelope SAC",
...
for method, exp in WS_EXPS.items():
    bar_data[method] = get_mean_std([exp])
